  ...
  _unwrapExternal(external, event) {
    return Promise.resolve()
      .then(() => {
        ...
        const keyBytes = otrKey.buffer;
        const referenceSha256 = sha256.buffer;
  ...
  _mapImageMedium(image, eventId) {
    ...
    return {
      data: {
        ...
        otr_key: new Uint8Array(image.otrKey ? image.otrKey.buffer : []),
        sha256: new Uint8Array(image.sha256 ? image.sha256.buffer : []),
      },
